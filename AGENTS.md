...
USE SIDECARS BY DEFAULT WHEN PARALLEL WORK CAN IMPROVE SPEED, QUALITY, OR SAFETY WITHOUT DUPLICATING THE CRITICAL PATH.
...
## Working agreements
- Behavioral coding doctrine:
- Think before coding: state assumptions, surface tradeoffs, and do not hide confusion.
- If multiple interpretations exist, name them; do not silently pick a risky branch.
- Simplicity first: write the minimum code that solves the verified problem; no speculative features, abstractions, configurability, or impossible-scenario handling.
- Surgical changes: touch only what the request requires, match existing style, and do not refactor adjacent code just because it is nearby.
...
- Goal-driven execution: turn every task into verifiable success criteria, implement against those criteria, and loop until fresh verification passes.
- For bug fixes, reproduce or encode the failure first when practical, then prove the fix.
- Prefer the simpler approach when it exists, and push back when a requested path would create needless complexity.
