...
async function withFakeCodex<T>(run: () => Promise<T>): Promise<T> {
  const fakeBinDir = await mkdtemp(path.join(tmpdir(), 'omx-retry-fake-codex-'));
...
async function createRetrySession(): Promise<SessionDocument> {
  return createSession({
    name: `OMX Retry ${Date.now()}`,
    source: 'manual',
    manifesto: 'Retry failed merge task.',
    architecture: 'Retry should requeue a failed task on the same build/workspace.',
    projectContext: 'retry contract',
    graph: {
      nodes: [
        {
          id: 'main-frontend',
...
async function seedFailedBuild(session: SessionDocument) {
  ...
  const buildId = 'retry-build-1';
  const workspacePath = path.join(runtimeDir, `build-${buildId}`);
  await rm(runtimeDir, { recursive: true, force: true }).catch(() => {});
  await import('node:fs/promises').then(async ({ mkdir, writeFile }) => {
    await mkdir(runtimeDir, { recursive: true });
    await mkdir(path.join(runtimeDir, 'workers'), { recursive: true });
    ...
    const executionGraph = {
      ...
      ownership: {
        ...
        rules: [
          ...
          { pathPattern: 'app/**', classification: 'shared-owner', ownerTaskId: 'task:main-frontend' },
          { pathPattern: 'components/**', classification: 'shared-owner', ownerTaskId: 'task:main-frontend' },
        ],
      },
    };
  ...
  try {
    ...
    await withFakeCodex(async () => {
      await withOmxServer(async (baseUrl) => {
        const response = await fetch(`${baseUrl}/api/omx/retry/${session.id}`, {
          method: 'POST',
        ...
        expect(response.status === 202, `Expected retry route to accept failed task. Got ${response.status}: ${text}`);
        expect(data?.buildId === seeded.buildId, `Expected retry to reuse buildId. Got: ${text}`);
        expect(data?.workspacePath === seeded.workspacePath, `Expected retry to reuse workspace. Got: ${text}`);
        expect(data?.activeWaveId === 'wave-1', `Expected retry to reactivate the failed wave. Got: ${text}`);
      });
    });
  } finally {
...
run().then(() => {
  console.log('PASS omx retry contract');
}).catch((error) => {
  console.error('FAIL omx retry contract');
