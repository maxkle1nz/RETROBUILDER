...
<product_realism_gate>
- Build the user-facing product, not a module diagnostic page, API demo, README visualizer, or pretty debug dashboard.
- Do not surface raw JSON, data contracts, acceptance criteria, module ids, generated-by labels, or developer scaffolding as UI.
- 21st references are mandatory design inputs when provided: adapt layout, spacing, motion, density, composition, and component behavior, not just copy their names into metadata.
- The Retrobuilder vanguard pattern database is the default taste contract. Use selected patterns across cinematic scroll/heroes, motion footers, COBE globe/map systems, controls (switches, sliders, comboboxes, selectors), tables/charts, story/chat surfaces, kinetic navigation, loading/state icons, shader/dither/ripple/canvas backgrounds, tooltips, accordions, 404s, media galleries, and trust proof strips as concrete design systems, not vague inspiration.
- Stack translation is mandatory: when the exact React/Tailwind/shadcn component cannot be used, recreate the closest equivalent in the available stack with native HTML/CSS/JS, Vue, Svelte, or server-rendered HTML while preserving motion story, composition, hierarchy, and mobile behavior.
- If a screenshot would not look credible beside a modern shipped product in the same domain, keep iterating before reporting completion.
- Mobile-first means a verified 390px viewport with no horizontal scrolling, clipped text, compressed controls, or unreadable payloads.
</product_realism_gate>
</constraints>
...
- Visible UI is the actual end-user product surface, not internal build/debug/status output
- Provided 21st/taste references visibly shape the layout, motion, density, and interaction model
- Selected Retrobuilder vanguard patterns are visibly translated into the implementation or intentionally approximated with stack-native primitives
