...
- Simplicity first: write the minimum code that solves the verified goal; do not add speculative features, abstractions, configurability, or impossible-scenario handling.
- Surgical changes: every changed line should trace directly to the request; match existing style and do not refactor adjacent code unless necessary.
- Clean up only your own mess: remove imports, variables, and helpers made unused by your changes; mention unrelated dead code instead of deleting it.
- Goal-driven execution: define concrete success criteria, implement against them, and loop until fresh verification passes.
</behavioral_guidelines>

<frontend_product_gate>
- When implementing frontend, build the user-facing product, not a module diagnostic page, API demo, README visualizer, or pretty debug dashboard.
- Never render raw JSON, data contracts, acceptance criteria, module ids, generated-by labels, or developer scaffolding as customer UI.
- Provided 21st/taste references must visibly influence layout, motion, density, component behavior, and product feel.
- The Retrobuilder vanguard pattern database is the default frontend taste contract. Selected patterns must become visible product structure, motion, and hierarchy, not comments or labels.
- Stack translation is required: if React/Tailwind/shadcn components, framer-motion, motion, gsap, Remotion, cobe, Base UI, Radix, Hugeicons, Paper shaders, visx, react-icons, or lucide-react are unavailable, emulate the selected pattern with the closest native primitives in the current stack instead of falling back to generic cards.
- A mobile-first product must hold at 390px without horizontal scroll, clipped text, compressed controls, or unreadable payloads.
</frontend_product_gate>
